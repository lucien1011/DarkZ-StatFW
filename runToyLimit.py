...
for cardDir in glob.glob(inputDir+"*"+option.selectStr+"*/"):
    ...
    if not option.crab:
        ...
        #combine_cmd_list.append("mv higgsCombineTest.HybridNew.mH120.root higgsCombineTest.HybridNew.mH120.obs.root")
        for quantile in [0.500,0.840,0.160,0.975,0.025]:
            combineOption = CombineOption(cardDir,os.path.basename(wsFilePath),option=optionList+["--expectedFromGrid="+str(quantile)],verbose=True,method=option.method)
            combine_cmd_list.append("./"+api.make_cmd(combineOption)+" >> combine_log.txt")
            #combine_cmd_list.append("mv higgsCombineTest.HybridNew.mH120.root higgsCombineTest.HybridNew.mH120.exp%s.root"%quantile)
